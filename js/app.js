...
courseFilter.addEventListener('change', renderRequirements); fundingFilter.addEventListener('change', renderRequirements);
...
function renderComparison() {
  const c1 = data.courses.find(c => c.id === document.getElementById('compare1').value); const c2 = data.courses.find(c => c.id === document.getElementById('compare2').value); const regime = document.getElementById('compareFunding').value; const r1 = c1.requirements[regime]; const r2 = c2.requirements[regime];
  document.getElementById('compareHead1').textContent = c1.name; document.getElementById('compareHead2').textContent = c2.name;
...
document.getElementById('calculatorForm').addEventListener('submit', event => {
  event.preventDefault(); const area = Number(document.getElementById('roomArea').value); const pc = Number(document.getElementById('pcCount').value); const lab = Number(document.getElementById('labCount').value);
  const results = data.courses.map(course => { const req = course.requirements.finanziato; const gaps = []; if (area < req.area) gaps.push(`Mancano ${req.area-area} mq`); if (pc < req.pc) gaps.push(`Mancano ${req.pc-pc} postazioni PC`); if (lab < req.lab) gaps.push(`Mancano ${req.lab-lab} postazioni laboratorio`); return {course,gaps}; });
...
function renderChecklistFilters() {
  const area = document.getElementById('checklistArea'); const status = document.getElementById('checklistStatus');
...
function renderChecklist() {
  const saved = JSON.parse(localStorage.getItem('janusChecklist') || '{}'); const area = document.getElementById('checklistArea')?.value || 'tutte'; const status = document.getElementById('checklistStatus')?.value || 'tutti'; const query = (document.getElementById('checklistSearch')?.value || '').trim().toLowerCase();
  const filtered = data.checklist.filter(item => { const haystack = [item.id,item.area,item.requirement,item.evidence,item.document,item.owner].join(' ').toLowerCase(); return (area==='tutte'||item.area===area)&&(status==='tutti'||item.status===status)&&(!query||haystack.includes(query)); });
  const available = data.checklist.filter(i=>i.status==='Disponibile').length; const partial = data.checklist.filter(i=>i.status==='Parziale').length; const linked = data.checklist.filter(i=>i.url).length;
  ...
  document.querySelectorAll('[data-check]').forEach(input => input.addEventListener('change', () => { saved[input.dataset.check]=input.checked; localStorage.setItem('janusChecklist',JSON.stringify(saved)); }));
}
document.getElementById('resetChecklist').addEventListener('click', () => { localStorage.removeItem('janusChecklist'); renderChecklist(); });
function renderSources() { document.getElementById('sourcesList').innerHTML = data.sources.map(source => `<article class="source-card"><h3>${source.title}</h3><div class="source-meta">${source.scope}</div><p>${source.note}</p><span class="badge">${source.status}</span></article>`).join(''); }

renderSidebarStats(); renderDashboard(); renderRequirements(); renderCompareControls(); renderComparison(); renderChecklistFilters(); renderChecklist(); renderSources();